...
async function drawRoute(points: MapPoint[]) {
  ...
  try {
    ...
    const routeCoordinates = await getOSRMRoute(coordinates)
    ...
    if (routeCoordinates.length > 0) {
      ...
      console.log(`Маршрут дня ${day + 1} построен: ${routeCoordinates.length} точек`)
    } else {
      ...
      console.log(`Fallback маршрут дня ${day + 1}: прямая линия`)
    }
  } catch (error) {
  ...
  <div class="routeMap">
    ...
    <div class="routeMap__info">
      ...
      <div class="routeMap__infoItem">
        ...
        <span class="routeMap__infoText">OSRM API (прямой вызов)</span>
      </div>
    </div>
    ...
    <div class="routeMap__hint">
      Нажмите на маркер, чтобы узнать подробности о месте. Маршруты строятся по настоящим дорогам.
    </div>
